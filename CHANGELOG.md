...
### Added
- **Event info & sponsors drawer** — new `ℹ` button in the header opens a right-side panel with the event description, route/distance/surface, dates, website + Instagram, emergency / organizer / technical phone numbers (as `tel:` links), and a 2-column grid of sponsor logos. Closes on ✕, backdrop click, or Escape.
- **Journals feed (new "Feed" tab)** — global reverse-chronological timeline of `PICTURE` (with 140×100 thumbnails linking to the full image) and `SLEEP` entries. Filter pills: All / Photos / Sleeps / ★ favorites. Clicking a rider name opens their detail view. Included in URL state (`?tab=feed`).
- **More notification types** — on top of CP crossed / caught by cactus / low battery:
  - **Passed the cactus** (behind → ahead).
  - **Rank gain ≥ 10 places** in a single refresh.
  - **Long stop ≥ 45 min** while the rider's last fix is still inside the rest block. Resets when they resume moving.
  - **New PICTURE** from a favourite, with the photo URL as the notification icon where supported.
  - Finish detection — CP notifications title distinguishes `reached CPn` vs `finished at <name>` for `FINISH`-type checkpoints.

### Fixed
- **UTC timestamp parsing** — upstream returns naive ISO strings (no `Z`). Added a `parseUtc` helper and routed every `new Date(str)` that consumes upstream timestamps through it. Corrects "since" text, stale-dot detection, event start/end math, cactus position, feed sort, and home-page classification for users whose browser isn't on UTC.

## [2026-04-20] 6d6e375 — More notifications

### Added
- Broader browser notification triggers beyond the initial set (iteration refined by the next "Unreleased" batch).

## [2026-04-20] bb61142 — Notifications on selected runners

### Added
- **Browser notifications** (tab-open only, permission-gated). 🔔 toggle in the event header. Fires for favourites on 30 s refresh diff:
...
- Dedup via unique `Notification.tag` per trigger + event.
...
### Added
- **Cactus delta on every leaderboard row** — `p.distance − cactus_km` → ±time and ±km vs the pacer, green/red/gray coloring. Honors the scrubber (rows re-render on playback changes when the list tab is visible).
...
### Added
- **Time scrubber + auto-play** (map tab) — range slider over `[event start, now]`, play/pause, speed dropdown (1s = 1min / 5min / 20min (default) / 1h / 6h), `live` jump button, localized time label. rAF-throttled redraws. Scrubbing pauses playback automatically.
...
- **Pale sand color** for the Cactus route polyline (upstream sent pure black).

## [2026-04-20] fdcd254 — Traces + dim cactus

### Added
- **Participant trace polylines** — the selected rider's trace is drawn bright; each favourite's trace is dimmer (colored by rank).
...
### Added
- **Category filter pills** in the sidebar (values read from `attributes.category` — on desertus-bikus-26: `F` femme, `H` homme, `M` mixte). Selecting a category filters the leaderboard and re-ranks by `p.rank` (the per-category rank) instead of `p.overall_rank`.
