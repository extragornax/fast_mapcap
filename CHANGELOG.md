...
### Added
- **Wind-flow overlay on the map** — new `wind` toggle in the map ⚙ popover (default off, persisted). Computes a 5×5 grid over the event's route bounding box (derived from the Cactus route geojson), fires a single batched Open-Meteo request for `wind_speed_10m` + `wind_direction_10m` at all 25 points, and renders each as a rotated `↑` divIcon with the km/h value, coloured by a Beaufort-ish band (calm / light / moderate / strong / gale). Cached 30 min. `pointer-events: none` so it doesn't steal marker clicks.
